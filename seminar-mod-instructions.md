...
3. The helper program can be configured to capture the voice input from the machine's microphone and a selected set of processes (intended to be a meeting software, so that the voices of both the speaker and the participants of a seminar can be recorded). These inputs/processes may be specified via CLA, and also via an interactive TUI if no CLA is provided. The helper program should then record audio from these inputs and processes and write them to files, with one file per audio source. These recording files should be written in stream to avoid data loss in case of an incident. 

   These recorded audio files will be used to be merged with board snapshot and history to produce a replay with audio. As such, these audio files should be marked with timestamps, either through file metadata or through separate metadata files. 

   - The server time and client time may differ. Thus, to remedy this difference, the helper program should try to periodically communicate with the server and sync time, and the timestamps for the recorded audio files should be server time. 
   - When creating a replay video, one or more audio recordings obtained in this way may be supplied via CLA. These audio recordings should be merged into the video. Now the time interval for the video creation and for each audio file may be different. When creating the video, for an audio file, only merge in audio files whose time intervals intersects the video's time interval (in server timestamp). If part of audio is outside the video time interval, just do not use that part in the video. If an audio started after the video started, just do not play that file until it is active. A similar treatment should be done to an audio that ended before the video ended. In other words, just place the part of each input audio file in the video time interval at the correct time moments in video.  -->
